...
interface TimerModalProps {
  ...
  onClose: () => void;
  timer?: Timer;
  timedFuntion: (title: string, description: string, totalSeconds: number) => void;
  modelType: string
}
...
export const TimerModal: React.FC<TimerModalProps> = ({
  ...
  const [hours, setHours] = useState(timer?.duration ? Math.floor(timer.duration / 3600): 0);
  const [minutes, setMinutes] = useState(timer?.duration ? Math.floor((timer.duration % 3600) / 60) : 0);
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setHours(timer?.duration ? Math.floor(timer.duration / 3600): 0);
      setMinutes(timer?.duration ? Math.floor((timer.duration % 3600) / 60) : 0);
      setSeconds(timer?.duration ? (timer.duration % 60) : 0);
      setTouched({
        title: false,
        hours: false,
        minutes: false,
      ...
      <div className="bg-white rounded-lg p-6 w-full max-w-md shadow-xl">
        <div className="flex justify-between items-center mb-6">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-xl font-semibold">{modelType === 'ADD' ? 'Add New Timer' : 'Edit Timer'}</h2>
          </div>
          <ModalButton 
            onClick={handleClose}
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex justify-end gap-3 pt-4 border-t">
            ...
            >
              {modelType === 'ADD' ? 'Add Timer' : 'Save Changes'}
            </ModalButton>
          </div>
        </form>
      </div>
